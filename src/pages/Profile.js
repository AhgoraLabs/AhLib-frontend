...
const ProfilePage = () => {
    ...
    const initialValue = { password: null }
    ...
    return (
        <div className='flex justify-around items-center'>
            <form className='flex flex-col rounded-xl h-3/4  mt-4 w-2/4' onSubmit={handleSubmit(onSubmit)}>
                <input className={styleInput} placeholder={'Nome de Usuário'} value={user.name} disabled={true} />
                <input className={styleInput} placeholder={'Email'} defaultValue={user.email} disabled={true} />
                <input className={styleInput} placeholder={'Nova senha'} {...register('password')} type={'password'} defaultValue={valueForm?.password} />
