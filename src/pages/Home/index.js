import _ from "lodash";
import moment from "moment";
import React, { useContext, useEffect, useState } from "react";
import { Chart } from "react-google-charts";
import { BiBookBookmark, BiCommentMinus, BiLike, BiUser } from "react-icons/bi";
import { BsStopwatch } from "react-icons/bs";
import { getUsers, listBooks } from "../../api/apiService";
import AuthContext from "../../context/auth/AuthContext";
import qrCode from "../../images/qrCodeApp.png";
import { Container, Dashboard } from './style';

const urlBase = !window.location.host.includes("netlify") ? "http://localhost:5000" : "https://ahlib.herokuapp.com";

function Home() {
    const { user: { name: usuario } } = useContext(AuthContext);


    const month1 = moment().format("MMM/YYYY");
    const month2 = moment().subtract(1, "months").format("MMM/YYYY");
    const month3 = moment().subtract(2, "months").format("MMM/YYYY");

    const [options, setOptions] = useState({});
    const [data, setData] = useState([
        ["Livros", "Quantidade"],
        ["Livros Livres", 0],
        ["Livros Alugados", 0],
    ]);

    const [users, setUsers] = useState([]);
    const [booksInfo, setBooksInfo] = useState({ totalBooks: 0, booksLate: 0 });
    const [loansInfo, setLoansInfo] = useState({ totalLoans: 0, loans2: 0 });



    //Essas options são keys padrões para modificar valores e visual do google charts
    const [option2, setOption2] = useState([
        [
            "Elemento",
            "Quantidade",
            { role: "style" },
            {
                sourceColumn: 0,
                role: "annotation",
                type: "number",
                calc: "stringfy",
                bars: "horizontal",
            },
        ],
        [month3, 0, "blue", null],
        [month2, 0, "green", null],
        [month1, 0, "gold", null],
    ]);

    async function loans() {
        const { data } = await (await fetch(`${urlBase}/loan`)).json();

        const group = data.map(loan => ({ date: moment(loan.createdAt, "YYYY-MM-DD").format("MMM/YYYY") }));

        const teste = Object.entries(_.groupBy(group, "date")).reduce((acc, [key, value]) => {
            acc[key] = value.length;
            return acc;
        }, {});


        var b = option2.map(([a, b, c, d]) => {
            if (teste[a]) b = teste[a];
            return [a, b, c, d];
        });
        setOption2(b);

        const booksHasntReturned = data.filter(({ bookHasReturned }) => !bookHasReturned);
        const booksLateAtTheMoment = booksHasntReturned.filter(book => new Date(book.newloanEnd || book.loanEnd) < new Date());

        setBooksInfo({
            ...booksInfo,
            booksLate: booksLateAtTheMoment.length,
            totalBooks: (await listBooks(8, 0)).data.count
        })

        setLoansInfo({
            ...loansInfo,
            totalLoans: booksHasntReturned.length,
            loans2: data
        })
    }

    useEffect(async () => {
        var b = data.map(([a, b]) => {
            if (a == "Livros Livres") b = booksInfo.totalBooks - loansInfo.totalLoans;
            if (a == "Livros Alugados") b = loansInfo.totalLoans;
            return [a, b];
        });
        setOptions({ title: `Total de Livros ${booksInfo.totalBooks}` });
        setData(b);
    }, [booksInfo.totalBooks, loansInfo.totalLoans]);

    useEffect(async () => {
        setUsers(await getUsers());
        loans();
    }, [])

    const options2 = {
        title: "Emprestimos por Periodo",
        bar: { groupWidth: "95%" },
        legend: { position: "none" },
    };

    return (
       <Container>
           <div id="header">
                <h1 id='title'>Olá, {usuario ? usuario.charAt(0).toUpperCase() + usuario.slice(1) : ''} !</h1>
                <h1 id='subtitle'>Bem vindo ao Ahlib</h1>
           </div>
           <Dashboard>
                <div id="right">
                    <div id="top">
                        <Chart bg-current width={"340px"} height={"220px"} chartType="PieChart" graphID="ScatterChart1" loader={<div>Carregando Painel</div>} data={data} options={options} />
                        <Chart bg-current chartType="BarChart" width={"340px"} height={"220px"} graphID="ScatterChart" fontSize={"100"} data={option2} options={options2} />
                    </div>
                    <div id="bottom">
                        <img
                            src="data:image/png;base64,"
                        ></img>
                    </div>
                </div>
                <div id="left">
                    <h1 id='title'>Indicadores Gerais</h1>
                    <div id="infos">
                        <h3><BiUser size={20} style={{ marginRight: 5 }} /> Usuários cadastrados: {users.length ? users.length : 0}</h3>
                        <h3><BiCommentMinus size={20} color="#494949" style={{ marginRight: 5 }} /> Recomendações: 0 </h3>
                        <h3><BiBookBookmark size={20} color="#494949" style={{ marginRight: 5 }} /> Emprestimos Totais: {loansInfo.loans2.length} </h3>
                        <h3><BsStopwatch size={20} color="#494949" style={{ marginRight: 5 }} />Livros em atraso: {booksInfo.booksLate}</h3>
                        <h3><BiLike size={20} color="#494949" style={{ marginRight: 5 }} />Livros avaliados: 0</h3>
                        <div id="qrcode">
                            <h1>Acesse pelo App</h1> <img alt="qrCode" src={qrCode} width={"170"} height={"170"} />
                        </div>
                    </div>
                    
                </div>
           </Dashboard>
       </Container>
    );
}

export default Home;
